...
export default function AniversarioPorteirinha() {
  const navigation = useNavigation();
  const [aniversarioPorteirinha, setaraniversarioPorteirinha] = useState([]);

  useEffect(() => {
    db.collection('aniversarioPorteirinha').onSnapshot((snapshot) => {
      setaraniversarioPorteirinha(
  ...
  return (
    <ScrollView style={{ backgroundColor: '#F8F5F5' }}>
      {aniversarioPorteirinha.map((val, index) => {
        if (index < 1) {
          return (
            <>
              <View style={{ flex: 1, marginTop: Platform.OS == 'ios' ? 50 : 50 }}>
                ...
                <View style={styles.containerimg}>
                  ...
                  <Text style={styles.titulo}>Desrição</Text>
