...
export default function DivinoEspiritoSanto() {
  const navigation = useNavigation();
  const [divinoEspiritoSanto, setardivinoEspiritoSanto] = useState([]);

  useEffect(() => {
    db.collection('divinoEspiritoSanto').onSnapshot((snapshot) => {
      setardivinoEspiritoSanto(
  ...
  return (
    <ScrollView style={{ backgroundColor: '#F8F5F5' }}>
      {divinoEspiritoSanto.map((val, index) => {
        if (index < 1) {
          return (
            <>
