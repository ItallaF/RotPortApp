...
export default function IgrejaSaoJose() {
  const navigation = useNavigation();
  const [igrejaSaoJose, setarigrejaSaoJose] = useState([]);

  useEffect(() => {
    db.collection('igrejaSaoJose').onSnapshot((snapshot: { docs: { map: (arg0: (doc: any) => { info: any; }) => React.SetStateAction<never[]>; }; }) => {
      setarigrejaSaoJose(
