...
export default function Cachoeira() {
  const navigation = useNavigation();
  const [cachoeira, setarcachoeira] = useState([]);

  useEffect(() => {
    db.collection('cachoeira').onSnapshot((snapshot: { docs: { map: (arg0: (doc: any) => { info: any; }) => React.SetStateAction<never[]>; }; }) => {
      setarcachoeira(
